package com.muhammad.network.vertexai
...
import com.muhammad.network.model.GeneratedPrompt
import com.muhammad.network.model.ImageValidationError
...
interface FirebaseAIDataSource{
    suspend fun validatePromptHasEnoughInformation(prompt : String) : ValidatedDescription
...
class FirebaseAIDataSourceImp(
    ...
    override suspend fun validatePromptHasEnoughInformation(prompt: String): ValidatedDescription {
        ...
        return executeTextValidation(
            generativeModel,
            "${remoteConfigDataSource.promptTextVerify()}. The input prompt is as follows:`$prompt`.",
        )
    }
